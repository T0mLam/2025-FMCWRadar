...
def parsePointCloudExtTLV(tlvData, tlvLength, outputDict):
    ...
    for i in range(numPoints):
        ...
        pointCloud[i,3] = 1 * pUnit[1]   # Doppler
...
#Parse 310 MMWDEMO_OUTPUT_EXT_MSG_MICRO_DOPPLER_RAW_DATA
def parseMicroDopplerRawDataTLV(tlvData, tlvLength, outputDict):
    try:
        arr = np.frombuffer(tlvData[:tlvLength], dtype=np.dtype('<f4')) 
    except Exception as e:
        log.error(f"MicroDopplerRaw parse failed: {e}")
...
#Parse 311 MMWDEMO_OUTPUT_EXT_MSG_MICRO_DOPPLER_FEATURES
def parseMicroDopplerFeaturesTLV(tlvData, tlvLength, outputDict):
    try:
        arr = np.frombuffer(tlvData[:tlvLength], dtype=np.dtype('<f4'))
    except Exception as e:
        log.error(f"MicroDopplerFeatures parse failed: {e}")
