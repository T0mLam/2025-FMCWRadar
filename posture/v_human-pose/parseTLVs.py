...
surfaceTypes = (0, "SURFACE_TYPE_WET_DRY", "SURFACE_TYPE_GRASS_NOTGRASS")
liquidClass = ("DRY", "WET")
grassClass = ("GRASS", "NOTGRASS")

def parseClassTLV(tlvData, tlvLength):
    classStruct = 'If'
    surfaceType, classificationResult = struct.unpack(classStruct, tlvData[:tlvLength])
    return surfaceTypes[surfaceType], liquidClass[int(classificationResult)]

def parseTrackTLV(tlvData, tlvLength):
    targetStruct = 'I9f16f2f'
    ...
    for i in range(numDetectedTargets):
        ...
        targets[i,10] = targetData[26] # G - gating function.
...
def parseGesturePresenceTLV6432(tlvData, tlvLength):
    presenceStruct = '1b'
